...
- [x] Configure package.json bin entry for `anytype` command
...
- [x] Create config module using `conf` package
- [x] Support storing: `apiKey`, `defaultSpace`, `aliases`
- [x] Config location: `~/.anytype-cli/config.json`
- [x] Add `getConfig()`, `setConfig()`, `hasConfig()` functions
...
- [x] Create typed API client with base URL configuration
...
- [x] Handle connection errors (Anytype not running)
...
- [x] Define exit code constants:
  ...
  - `4` - Not found
  - `5` - Validation error
- [x] Create error classes for each type
...
- [x] Initialize Commander.js program
- [x] Add global flags: `--json`, `--verbose`, `--space`
- [x] Set up command routing structure
...
- [x] Prompt for API key (or accept via flag)
- [x] Validate API key by calling `/v1/spaces`
- [x] Fetch available spaces and prompt for default
...
- [x] Fetch types from `/v1/spaces/{space_id}/types`
- [x] Display type name, key, and property count
- [x] Support `--json` output
- [x] Cache types locally for faster alias resolution
...
- [x] Resolve type aliases (e.g., `1on1` → actual type key)
- [x] Fetch objects from `/v1/spaces/{space_id}/objects?type_key=X`
- [x] Display as markdown table: name, id, updated date
- [x] Support flags:
...
- [x] Support lookup by ID or name
- [x] Fetch full object including body
- [x] Format markdown body for terminal display
- [x] Show linked objects/relations
- [x] Support `--json` output
...
- [x] Call global search endpoint
- [x] Support `--type` filter
- [x] Support `--limit` and `--offset` for pagination
- [x] Display results as markdown list
...
- [x] Accept name as positional argument or flag
- [x] Support `--body` for markdown content
- [x] Support `--body-file` to read from file
- [x] Support `--property key=value` for properties (repeatable)
- [x] Support `--link-to <id>` to create relations
- [x] Implement `--dry-run` to preview without creating
- [x] Return created object ID on success
...
- [x] Lookup object by ID or name
- [x] Support `--name` to rename
- [x] Support `--body` to replace body
- [x] Support `--append` to append to body
- [x] Support `--property key=value` to update properties
- [x] Support `--link-to` and `--unlink-from` for relations
...
- [x] Validate type exists before create
- [x] Validate required properties for type
- [x] Validate date formats
- [x] Validate property values against type schema
...
- [x] Add `--fields` flag to select specific properties
- [x] Add `--format` flag for custom output templates
- [x] Improve markdown table rendering
- [x] Add color coding for types and status
...
- [x] Generate comprehensive help for each command
- [x] Add examples to help text
- [x] Create `anytype help` as alias for `--help`
- [x] Add `anytype help <command>` support
...
- [x] Implement `anytype list notes --orphan` flag
- [x] Detect notes with no links and no backlinks
- [x] Optionally filter by age (`--older-than 7d`)
...
- [x] Support reading body from stdin
- [x] Enable piping: `echo "content" | anytype create note "Title"`
- [x] Support `--body -` to read from stdin
